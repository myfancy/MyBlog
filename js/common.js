...
/*
 ...
 3）http://www.xxxx.com/about.html[?id=item3]#item5?nojump   ([]代表可选)
 第一种结果，是页面默认情况，自动打开第一个选项卡，不用处理，并且在正式开发中，菜单栏的链接不会这么写的
 第二种结果，是菜单栏的链接的格式。之所以不用#id?nojump，是因为这样页面不会刷新。但因为菜单栏也没有关联下面的tab点击事件，所以会出现第一次点击这种链接是如公司简介（……#introduce?nojump）时，页面刷新打开；然后再次点击，如资质荣誉(……#honor?nojump)时,页面会没有反应。故这里采用了(?id=)的方式传参，保证页面会刷新。
 第三种结果，是点击tab标签后修改hash用来保存标签打开情况产生的值，加?nojump是为了不触发锚点跳转事件。（原理：随便加个后缀，使浏览器找不到指定id就不跳转了）。之所以修改hash不修改href，是因为一旦修改href则页面必刷新。
 得到这三种结果后，1)无需处理，2)3)两种情况，如果有?nojump,说明在做了切换标签的操作，就以它为准，若无，以?id=为准。然后通过js的一个方法截取字符串，根据得到的值打开指定的标签。

 说明2：传参
 1）?id= 菜单栏的连接是通过后台传的值，无需处理。
 2）#id?nojump 需要编写js事件，在点击tab的时候修改页面hash
 ...
 解决办法：把地图放到tab的下面，默认隐藏，页面刷新或点击tab时，检测到打开的是【联系我们】即显示。
...
$(document).ready(function(){
    ...
    //url可能是 (?tabID=item1111) 或 ([?tabID=item1111]#item2222?nojump)
    //当有(?nojump)存在时，以 #item2222 为准，打开指定标签。
    //当无(?nojump)但有(?id=)时，以 #item1111 为准，打开指定标签。（若两者都无，无需做任何处理，默认打开第一个标签）
